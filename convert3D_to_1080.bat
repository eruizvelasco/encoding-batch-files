@ECHO OFF
REM ** convert3D_to_1080 batch file **
REM
...
REM then scales to 1920x1080 and encodes the file using ffmpeg and moves the 3D file moves to <drive>:\3D directory.
...
IF NOT EXIST "%~d1%~pn1.%~x1" (
	...
	ffmpeg -y -vsync 0 -hwaccel cuvid -c:v h264_cuvid -i %1 -c:a copy -vf scale_npp=1920:1080 -c:v h264_nvenc -profile:v main -preset slow -rc vbr_hq -rc-lookahead 32 -b:v 9M -c:a copy -b:a 500k "%~d1%~pn1_1080p%~x1" > CON	
	IF NOT errorlevel 1 (
		IF NOT EXIST "%~d1\3D%~p1" (mkdir "%~d1\3D%~p1")
		ECHO Moving file %1 to "%~d1\3D%~p1" > CON
		move /Y %1 "%~d1\3D%~p1"
	)
)
del "%~d1%~pn1.txt"
