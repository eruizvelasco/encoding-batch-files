...
REM
REM This file process all 4K mp4 files in a directory and finds the files with 4096 or 3840 resolution
REM then encodes to HEVC using ffmpeg and moves the 4K file moves to <drive>:\4K directory.
...
IF NOT EXIST "%~d1%~pn1.%~x1" (
	...
	ffmpeg -y -vsync 0 -hwaccel cuvid -c:v h264_cuvid -i %1 -c:a copy -vf scale_npp=1920:1080 -c:v h265 -b:v 50M -b:a 50M "%~d1%~pn1.mkv" > CON
	IF NOT errorlevel 1 (
