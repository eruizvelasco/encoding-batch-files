...
IF NOT EXIST "%~d1%~pn1.%~x1" (
	...
	ffmpeg -y -vsync 0 -hwaccel cuvid -c:v h264_cuvid -i %1 -vf scale_npp=1920:1080 -c:v h264_nvenc -b:v 12M -profile:v main -preset slow -rc vbr_hq -rc-lookahead 32 -c:a copy "%~d1%~pn1_1080p.%~x1" > CON
	IF NOT errorlevel 1 (
