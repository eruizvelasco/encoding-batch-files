...
IF NOT EXIST "%~d1%~pn1.%~x1" (
	...
	REM ffmpeg -y -vsync 0 -hwaccel cuvid -c:v h264_cuvid -i %1 -c:a copy -vf scale_npp=1920:1080 -c:v h264_nvenc -b:v 50M -b:a 50M "%~d1%~pn1_1080p.%~x1" > CON
	ffmpeg -y -vsync 0 -hwaccel cuvid -c:v h264_cuvid -i %1 -vf scale_npp=1920:1080 -c:v h264_nvenc -b:v 12M -profile:v main -preset slow -rc vbr_hq -rc-lookahead 32 -c:a aac -b:a 448k "%~d1%~pn1_1080p.%~x1" > CON
	IF NOT errorlevel 1 (
		IF NOT EXIST "%~d1\original%~p1" (mkdir "%~d1\original%~p1")
		ECHO Moving file %1 to "%~d1\original%~p1" > CON
		move /Y %1 "%~d1\original%~p1"
	)
)
del "%~d1%~pn1.txt"
EXIT /B
